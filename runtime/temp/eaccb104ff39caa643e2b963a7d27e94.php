<?php /*a:5:{s:53:"D:\wamp64\www\a\application\admin\view\point\lst.html";i:1575172831;s:56:"D:\wamp64\www\a\application\admin\view\public\_head.html";i:1573053339;s:56:"D:\wamp64\www\a\application\admin\view\public\_left.html";i:1573144658;s:59:"D:\wamp64\www\a\application\admin\view\public\_navtree.html";i:1573049068;s:56:"D:\wamp64\www\a\application\admin\view\public\_foot.html";i:1574854748;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
  ...
  <div class="page-body">
    <div class="widget">
      <header class="head clearfix">
        <a href="<?php echo url('admin/point/add'); ?>" class="btn fl">添加</a>
        <form class="serachform" id="serach" action="<?php echo url('admin/point/lst'); ?>" method="post">
...
<script>
  $('.booklst').select2({
    ...
    width: 'resolve'
        ...
        <form class="serachform" id="serach" action="<?php echo url('admin/point/search'); ?>" method="post">
       ...
       <div class="body">
         <table class="table">
          <thead>
           ...
           <tbody>
          <?php if(is_array($pointlst) || $pointlst instanceof \think\Collection || $pointlst instanceof \think\Paginator): $i = 0; $__LIST__ = $pointlst;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
          <tr>
            <td class="text-center"><?php echo htmlentities($vo['id']); ?></td>
            <td><a href="<?php echo url('admin/point/edit', ['id' => $vo['id']]); ?>"><?php echo htmlentities($vo['title']); ?></a></td>
            <td><a href="<?php echo url('admin/point/lst',['id' => $vo['book_id']]); ?>"><?php echo htmlentities($vo['Pointcate']['title']); ?></a></td>
             <td class="text-center">
              <?php if($vo['atop'] == 1): ?>
               <i class="icon atop i-checkactive" dataid="<?php echo htmlentities($vo['id']); ?>" atop="<?php echo htmlentities($vo['atop']); ?>"></i>
                <?php else: ?>
                <i class="icon atop i-check" dataid="<?php echo htmlentities($vo['id']); ?>" atop="<?php echo htmlentities($vo['atop']); ?>"></i>
                <?php endif; ?>
             </td>
             <td class="text-center"><?php echo htmlentities(date("m-d H:i:s",!is_numeric($vo['create_time'])? strtotime($vo['create_time']) : $vo['create_time'])); ?></td>
            <td class="text-center">
              <a href="<?php echo url('index/point/art', ['id' => $vo['id']]); ?>" target="_Blank">预览</a>
              <a href="<?php echo url('admin/point/edit', ['id' => $vo['id']]); ?>">编辑</a>
...
$(function () {
  ...
  $('.atop').click(function () {
    ...
    $.ajax({
      url:"<?php echo url('admin/point/atop'); ?>",
      type:'post',
...
//删除
 $('.delete-btn').click(function (ev) {
    ...
    var id = $(this).val();
    $.ajax({
       url:"<?php echo url('admin/point/del'); ?>",
       type:'post',
       data:{id:id},
       dataType:'json',
       success:function (data) {
